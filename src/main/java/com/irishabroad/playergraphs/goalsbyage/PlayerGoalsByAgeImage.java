...
public class PlayerGoalsByAgeImage {
	...
	public PlayerGoalsByAgeImage(@Nonnull List<String> names, @Nonnull List<Integer> goals, int years, int months) {
	...
	public void generateImage(int counter) throws IOException {
		...
		graphics.drawString(monthString, 880 - fontMetrics.stringWidth(monthString), 320);
		...
		image = new File("output/capsbygoals/playerGoals" + String.format("%03d", counter) + ".png");
		if(image.exists()) {
